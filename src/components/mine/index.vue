<template>
  <div class="mine-wrap">
    ...
    <div v-show="msg" class="mine-ul">
        <ul v-for="(item,index) in userInfos" :key="index">
            ...
            Axios.get('/upDateUserInfo',{
              ...
              let data=res.data.data
                 this.userInfos.push(data)
                  window.localStorage.setItem('userInfo',JSON.stringify({id:data.id,username:data.username,phone:data.phone,avator:data.avator}))
            })
        },
    setAvator(data){
      if(data===null){
        this.avator='https://ss0.bdstatic.com/70cFuHSh_Q1YnxGkpoWK1HF6hhy/it/u=9279531,4278322843&fm=27&gp=0.jpg'
      }else{
