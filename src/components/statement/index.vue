 ...
 * @Date: 2019-02-27 14:50:00 
 * @Last Modified by: mikey.liujiaxu
 * @Last Modified time: 2019-02-27 15:03:33
 */
/*
 * @Author: mikey.zhaopeng 
 * @Date: 2019-02-22 11:21:11 
 * @Last Modified by: mikey.liujiaxu
 * @Last Modified time: 2019-02-27 14:48:51
 */
<template>
  <div class="wrap">
...
export default {
  ...
  computed: {
    initPirce() {
      ...
      let datas = Object.values(this.$store.state.data);
      data.map(item => {
      ...
      datas.map(item => {
        if (item.checked === true) {
          nums += item.dayRent * item.count;
        }
      });
      return (num + nums).toFixed(2);
    }
  },
  methods: {
    pay() {
      const { state, commit } = this.$store;
      commit("cont", this.initPirce);
